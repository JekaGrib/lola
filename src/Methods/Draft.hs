...
getDrafts h usIdNum (GetDrafts pageNum) = do
  ...
  drafts <- checkListE h $ selectLimitDraft h table orderBy pageNum (cDraftsLimit . hConf $ h) extractParams where' values [] []
...
getDraftInfo h usIdNum (DraftRequest _ _ catIdParam _ picId picsIds tagsIds) = do
  ...
  tagS <- checkListE h $ selectTag h "tags" ["tag_id","tag_name"] where' (fmap (pack . show) tagsIds)
