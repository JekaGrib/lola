...
deleteDbUser' conn usId = do
  let wh = WherePair "user_id=?" (Txt usId)
  deleteFromDb' conn (Delete "users" wh)
deleteDbAuthor' conn auId = do
  let wh = WherePair "author_id=?" (Txt auId)
