...
tryReadNumArray paramKey txt = case reads . unpack $ txt of
  [([], "")] ->
    throwE $ BadReqError $
      "Can`t parse parameter: " ++ unpack paramKey
        ++ ". It must be NOT empty array of numbers. Example: [3,45,24,7] "
  [(a, "")] -> return a
  _ ->
    throwE $ BadReqError $
      "Can`t parse parameter: " ++ unpack paramKey
        ++ ". It must be array of numbers. Example: [3,45,24,7] "
...
tryReadDay paramKey txt = do
  ...
  case filter (' ' /=) . unpack $ txt of
    ...
    [a, b, c, d, '-', e, f, '-', g, h] -> do
      ...
      when (month `notElem` [1 .. 12]) $ throwE $ BadReqError $
        "Can`t parse parameter: " ++ unpack paramKey
          ++ ". Month must be a number from 1 to 12."
          ++ infoErrStr
      day <- tryReadInteger paramKey (pack [g, h]) `catchE` addToBadReqErr infoErrStr
      when (day `notElem` [1 .. 31]) $ throwE $ BadReqError $
        "Can`t parse parameter: " ++ unpack paramKey
          ++ ". Day of month must be a number from 1 to 31."
          ++ infoErrStr
    ...
    _ ->
      throwE $ BadReqError $
        "Can`t parse parameter: " ++ unpack paramKey ++ infoErrStr

tryReadSortOrd :: (Monad m) => QueryParamKey -> Text -> ExceptT ReqError m SortOrd
tryReadSortOrd paramKey "" =
  throwE $ BadReqError $
    "Can`t parse parameter: " ++ unpack paramKey
      ++ ". Empty input."
...
checkBigIntId paramKey num
  ...
  | num > 9223372036854775805 =
    throwE $ BadReqError $
      "Parameter: " ++ unpack paramKey
        ++ ". Id should be less then 9223372036854775805"
