...
6. Draft methods:
         ...
         b) To create draft for post:
           POST posts/post_id INT/drafts
           User should be post author.
           Query parameters:
            ▪ token TXT (user/admin token)
           
         c) To publish draft (create post(if it is new draft) or update post(if it is post`s draft)):
           POST drafts/draft_id INT/posts
           User should be draft author.
           Query parameters:
            ▪ token TXT (user/admin token)
           answer example (for update post):
         ...
         e) To get several drafts for author:
           GET /drafts
         ...
         f) To update draft:
           ...
           example : «http://localhost:3000/updateDraft/241»
           User should be draft author.
           Query parameters:
            ▪ token TXT (user/admin token)
           JSON parameters:
